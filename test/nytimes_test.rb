...
class NytimesTest < Minitest::Test
  ...
  def test_it_can_get_copyright
    anything = @hash[:copyright]

    assert anything, "Copyright (c) 2018 The New York Times Company. All Rights Reserved."
  end

  def test_it_can_get_array_of_stories
    something = @hash[:results]

    assert something.is_a? (Array)
    assert_equal 44, something.count
  end
